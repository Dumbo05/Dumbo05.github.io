...
export default async function Article({params}:{params:Promise<{slug:string}>}){
 ...
 return <><a className="b-back" href={`${base}/blog/`}>← 返回全部文章</a><header className="b-article-hero"><img src={`${base}${post.cover}`} alt=""/><div/><section><span>{post.category}</span><h1>{post.title}</h1><p>Dumbo05 · <time dateTime={post.date}>{post.date}</time> · 约 {Math.max(1,Math.ceil(post.body.length/400))} 分钟阅读</p></section></header>
 <div className="b-columns b-reading-columns"><article className="b-card b-article"><div className="b-article-lead">{post.description}</div><Markdown body={post.body}/><div className="b-article-bottom"><p>作者：Dumbo05</p><p>本文链接：<a href={postUrl(post.slug)}>dumbo05.github.io/blog/{post.slug}/</a></p><span>{post.tags.map(t=>`#${t}`).join(' · ')}</span></div><a className="b-bottom-back" href={`${base}/blog/`}>← 回到博客，继续探索</a></article>
