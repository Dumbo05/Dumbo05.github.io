...
export default function BlogSidebar(){ return <aside className="b-sidebar">
  <section className="b-author"><div className="b-author-top">HELLO, FRIEND.</div><img src={`${base}/avatar.jpg`} alt="Dumbo 的山峰头像" width="86" height="86"/><h2>Dumbo</h2><p>在代码里探索，<br/>在文字里留下足迹。</p><div className="b-counts"><a href={`${base}/blog/`}><strong>{posts.length}</strong>文章</a><a href={`${base}/blog/archive/`}><strong>{new Set(posts.map(p=>p.category)).size}</strong>分类</a><a href={`${base}/blog/archive/`}><strong>{new Set(posts.flatMap(p=>p.tags)).size}</strong>标签</a></div><a className="b-author-link" href="https://github.com/Dumbo05">在 GitHub 找到我 ↗</a></section>
